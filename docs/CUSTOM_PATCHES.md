...
Verified Compact support is a preference only inside the same administrator
priority layer. It cannot move a higher-cost lane ahead of the price order
configured in the panel. Within that layer, verified lanes are tried before
unknown or legacy-failure lanes; if they are full or fail, the latter remain
available as a soft fallback. This applies consistently to both the advanced
load-aware scheduler and the legacy fallback path.
