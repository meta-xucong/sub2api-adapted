export default {
    settings: {
      ...
      site: {
        ...
        apiBaseUrl: '站点根地址（API 与回调）',
        apiBaseUrlHint: '请输入不带路径的站点域名，例如 https://api.example.com。用户端 OpenAI/Codex/Grok 地址会显示为 /v1，管理 API 与回调使用 /api/v1；留空则使用当前站点地址。',
        ...
        customEndpoints: {
          title: '自定义端点',
          description: '添加额外的完整 API 基地址，用户可在「API Keys」页面快速复制。OpenAI 兼容端点请包含 /v1；其他协议请填写其实际路径。',
          itemLabel: '端点 #{n}',
          name: '名称',
          namePlaceholder: '如：OpenAI Compatible',
          endpointUrl: '完整 API 基地址',
