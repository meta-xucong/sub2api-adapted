        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-green-600 dark:text-green-400">⭐️{{ row.actual_cost?.toFixed(6) || '0.000000' }}</span>
              <!-- Cost Detail Tooltip -->
              <div
                class="group relative"
            ...
            <div v-if="row.account_rate_multiplier != null" class="mt-0.5 text-[11px] text-orange-500 dark:text-orange-400">
              A ⭐️{{ accountBilled(row).toFixed(6) }}
            </div>
          </div>
        </template>
      ...
      <div class="whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-gray-600 dark:bg-gray-800">
        <div class="space-y-1.5">
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-green-400">⭐️{{ tooltipData?.actual_cost?.toFixed(6) || '0.000000' }}</span>
          </div>
