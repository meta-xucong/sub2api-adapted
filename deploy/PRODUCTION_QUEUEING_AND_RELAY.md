...
This note captures two production hardening patterns that are easy to lose during upgrades:

1. OpenAI image-generation permission checks should tolerate stale auth cache.
...
## 1. OpenAI Image Permission Cache Recovery

If a group's image-generation permission is enabled in storage but the auth cache is stale, image requests may still be rejected with `permission_error`.

The adapted codebase adds a slow-path auth refresh:
...
- First, use the fast cached auth view.
- If image generation appears disallowed, reload the latest API key auth view from storage.
- If the refreshed group allows image generation, continue the request and refresh the request context.

This keeps normal requests fast while making permission flips take effect without waiting for cache expiry.
