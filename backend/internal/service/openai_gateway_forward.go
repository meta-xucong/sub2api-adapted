...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if compactPath {
		// The legacy /responses/compact gateway contract is unary even when the
		// probe body carries the native compaction trigger's stream=true hint.
		// Keep the upstream body intact, but parse the returned compact document
		// through the non-streaming response path.
		reqStream = false
	}
	...
	if compactPath {
		// Re-apply after account-specific body normalization, which may rebuild
		// requestView and restore the client's stream=true hint.
		reqStream = false
	}
