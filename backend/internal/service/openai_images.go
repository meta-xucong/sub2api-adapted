...
// validateOpenAIImagesRequestModel also accepts provider-specific image model
// names that are routed through the OpenAI-compatible Images API surface.
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	if resp.StatusCode >= 400 {
		...
		_ = resp.Body.Close()
		respBody = s.redactAgentIdentitySensitiveBody(imageUpstreamCtx, account, respBody)
		...
		return s.handleOpenAIImagesErrorResponse(imageUpstreamCtx, resp, c, account, upstreamModel)
	}
	...
	if parsed.Stream && isEventStreamResponse(resp.Header) {
		...
		nonStreamUsage, nonStreamCount, nonStreamSizes, err := s.handleOpenAIImagesNonStreamingResponse(imageUpstreamCtx, resp, c, account, parsed)
		if err != nil {
			return nil, err
		}
