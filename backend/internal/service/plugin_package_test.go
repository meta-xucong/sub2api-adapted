...
func TestPluginPackageInstallerInstallUnsignedDevelopmentPackage(t *testing.T) {
	...
	// Windows does not expose POSIX executable bits through os.FileMode; the
	// installer still writes the binary and the host launches it via the native
	// platform rules. Keep the permission assertion on platforms where it has
	// observable meaning.
