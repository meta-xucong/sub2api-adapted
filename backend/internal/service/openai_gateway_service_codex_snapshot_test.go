...
func TestBuildCodexUsageExtraUpdates_ZeroFiveHourUsedStaysZero(t *testing.T) {
	primaryUsed := 0.0
	primaryReset := 18000
	primaryWindow := 300
	secondaryUsed := 33.0
	secondaryReset := 172800
	...
	snapshot := &OpenAICodexUsageSnapshot{
		...
		SecondaryResetAfterSeconds: &secondaryReset,
		SecondaryWindowMinutes:     &secondaryWindow,
		UpdatedAt:                  "2026-06-09T10:31:01+08:00",
	}
	...
	if got := updates["codex_7d_used_percent"]; got != 33.0 {
		t.Fatalf("codex_7d_used_percent = %v, want 33", got)
	}
}
