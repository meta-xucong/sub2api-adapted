...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if !isOpenAIAccountSchedulableForRequest(ctx, account, req.RequestedModel, req.RequireCompact) || account.Platform != normalizeOpenAICompatiblePlatform(req.Platform) || !account.IsOpenAICompatible() {
			continue
		}
...
// SelectAccountWithSchedulerForImageOperation distinguishes generation from edits
// so Smart Router can keep the two capability lanes isolated.
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImageOperation(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, false, PlatformOpenAI, false, smartCapability)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		selection, decision, err = s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, PlatformOpenAI, false, smartCapability)
		if err == nil && selection != nil && selection.Account != nil {
			return selection, decision, nil
		}
		if s.shouldRetryOpenAIImageSelectionWithFreshAccounts(ctx, err) {
			return s.selectAccountWithScheduler(withOpenAIBypassSchedulerSnapshot(ctx), groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, PlatformOpenAI, false, smartCapability)
		}
		return selection, decision, err
	}
	if s.shouldRetryOpenAIImageSelectionWithFreshAccounts(ctx, err) {
		return s.selectAccountWithScheduler(withOpenAIBypassSchedulerSnapshot(ctx), groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, false, PlatformOpenAI, false, smartCapability)
	}
	return selection, decision, err
}
