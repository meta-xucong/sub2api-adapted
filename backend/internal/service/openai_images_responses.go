...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	imageUpstreamCtx = withOpenAIImagesSelfBuiltRequest(imageUpstreamCtx)
	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) {
			...
			shouldDisable := s.handleFailoverSideEffects(imageUpstreamCtx, resp, account, respBody, requestModel)
