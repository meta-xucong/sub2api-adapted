...
func (i *PluginPackageInstaller) Install(ctx context.Context, reader io.Reader, installedBy *int64) (*PluginInstallation, error) {
	...
	// zip.OpenReader keeps the package file open on Windows. Close it before
	// atomically moving the uploaded artifact into packages/, otherwise the
	// move fails with a sharing violation even though all archive entries have
	// already been read.
